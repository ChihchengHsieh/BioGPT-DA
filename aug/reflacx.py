...
def get_prompt(mimic_eye_path, data, label_cols, feature_to_name, desired_clinical_feature, report_format = False):
    # reflacx_id = data['id']
    patient_id = data['subject_id']
    study_id = data['study_id']
    # dicom_id = data['dicom_id']
    report_path = os.path.join(
        mimic_eye_path, f"patient_{patient_id}", "CXR-DICOM", f"s{study_id}.txt",
    )
    ...
    if report_format:
        return re.sub('[^0-9A-Za-z.\s\:\']', '', f"{report} LESIONS:{get_diagnosis(data, label_cols)}. AGE: {age}. GENDER: {gender}. {feature_to_name[desired_clinical_feature]}: ")
    else:
        ...
        return re.sub('[^0-9A-Za-z.\s\:\']', '', f"A {age} years old {gender} patient diagnosed with{get_diagnosis(data, label_cols)}. And, This patient has the radiology report: \n{report}\nThe {feature_to_name[desired_clinical_feature]} of this patient is around")
...
def aug_df(mimic_eye_path, label_cols, features_to_aug, feature_to_name_map, df, generator, progress=[1, 5, 25, 50], report_format=False):
    ...
    for f in features_to_aug:
        ...
        for idx, data in tqdm(df.iterrows(), total=df.shape[0]):
            prompt = get_prompt(mimic_eye_path, data, label_cols, feature_to_name_map, f, report_format = report_format)
            v = progressive_aug(generator, prompt, aug_feature_range[f], progress = progress)
            if v is None:
                print(f"Couldn't find value for [{idx}] prompt [{progress[-1]} sequences]: {prompt}")
